...
export default function GroupBehavior(eventBus, bpmnFactory, canvas, elementRegistry) {
  ...
  /**
   * Updates label of each group which references same categoryValue,
   * if necessary
   */
  this.postExecuted('element.updateLabel', LOW_PRIORITY, function(event) {

    var context = event.context,
        shape = context.element,
        categoryValueRef = getBusinessObject(shape).categoryValueRef;

    if (is(shape, 'bpmn:Group') && categoryValueRef) {
      ...
      groupElements.forEach(g => {
        ...
        if (g === shape
          ||!label
          || !isReferenced([ g ], categoryValueRef)) {
          return;
        }
        var gfx = elementRegistry.getGraphics(label),
            textContent = gfx.textContent;

        if (textContent !== newLabel) {
