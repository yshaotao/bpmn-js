 ...
 * Remove root elements once they are not being referenced anymore.
 * Copy referenced root elements on copy & paste.
 */
export default function RootElementReferenceBehavior(bpmnjs, elementRegistry, eventBus, injector) {
  ...
  function isRootElementReferenced(rootElement) {
    var events = elementRegistry.filter(function(element) {
      return hasEventDefinition(element, getReferencingEventDefinitionType(rootElement));
    });

    return some(events, function(event) {
      var businessObject = getBusinessObject(event),
          eventDefinitions = businessObject.get('eventDefinitions'),
          eventDefinition = eventDefinitions[ 0 ];

      return getRootElementReferenced(eventDefinition) === rootElement;
    });
  }
  ...
  this.executed('shape.delete', function(context) {
    ...
    if (rootElement
        ...
        && !isRootElementReferenced(rootElement)) {
      rootElements = bpmnjs.getDefinitions().get('rootElements');

      // remove root element
      collectionRemove(rootElements, rootElement);

      context.removedRootElement = rootElement;
    }
  }, true);

  this.reverted('shape.delete', function(context) {
    var removedRootElement = context.removedRootElement;

    if (!removedRootElement) {
      return;
    }
    ...
    // add root element
    collectionAdd(rootElements, removedRootElement);
  }, true);
...
function getReferencingEventDefinitionType(rootElement) {
  if (is(rootElement, 'bpmn:Error')) {
    return 'bpmn:ErrorEventDefinition';
  } else if (is(rootElement, 'bpmn:Escalation')) {
    return 'bpmn:EscalationEventDefinition';
  } else if (is(rootElement, 'bpmn:Message')) {
    return 'bpmn:MessageEventDefinition';
  } else if (is(rootElement, 'bpmn:Signal')) {
