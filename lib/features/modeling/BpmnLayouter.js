...
BpmnLayouter.prototype.layoutConnection = function(connection, hints) {
  ...
  if (is(connection, 'bpmn:MessageFlow')) {
    ...
    if (hints.preview && !hints.connectionStart && hints.connectionEnd) {
      var adjustedStart = assign({}, start, { x: hints.connectionEnd.x });

      if (pointInRect(adjustedStart, source, -7)) {
        start = adjustedStart;
      } else {
        manhattanOptions.preferredLayouts = [ 'v:v' ];
      }
