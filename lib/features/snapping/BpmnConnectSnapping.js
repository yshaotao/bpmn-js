  ...
  eventBus.on([
    ...
    if (target && isAnyType(connectionAttrs, [
      ...
      if (isAny(target, [ 'bpmn:Task', 'bpmn:SubProcess' ])) {
        snapToTargetMid(event, source, target);
      }
...
// snap to target mid
function snapToTargetMid(event, source, target) {
  var sourceMid = mid(source),
      targetMid = mid(target);

  var orientation = getOrientation(sourceMid, target);

  AXES.forEach(function(axis) {
    if (shouldSnapToTargetMid(event, source, target, orientation)) {
...
/**
 * Check whether event should snap to target mid.
 *
 ...
 * @returns {boolean}
 */
function shouldSnapToTargetMid(event, source, target, orientation) {
  var sourceIsBoundaryEvent = is(source, 'bpmn:BoundaryEvent'),
      targetMidBounds = asTRBL(target);

  if (orientation.includes('top') ||
    (sourceIsBoundaryEvent && orientation.includes('left'))) {
    targetMidBounds.top += TARGET_MID_PADDING;
  }

  if (orientation.includes('right') ||
  (sourceIsBoundaryEvent && orientation.includes('bottom'))) {
    targetMidBounds.right -= TARGET_MID_PADDING;
  }

  if (orientation.includes('bottom') ||
  (sourceIsBoundaryEvent && orientation.includes('left'))) {
    targetMidBounds.bottom -= TARGET_MID_PADDING;
  }

  if (orientation.includes('left') ||
  (sourceIsBoundaryEvent && orientation.includes('bottom'))) {
    targetMidBounds.left += TARGET_MID_PADDING;
  }

  return pointInRect(event, asBounds(targetMidBounds));
}