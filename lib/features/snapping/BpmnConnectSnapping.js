...
export default function BpmnConnectSnapping(eventBus, rules) {
  eventBus.on([
    ...
    var context = event.context,
        source = context.source || context.hover,
    ...
    if (!context.originalStartPosition) {
      context.originalStartPosition = context.startPosition;
    }

    // TODO(nikku): swap ->
    //   * we should not snap on hover and rely on pre-computed context.canExecute
    ...
    if (target && isAnyType(connectionAttrs, [
      ...
      // snap source
      context.startPosition = mid(source);
      ...
      // un-snap source
      context.startPosition = context.originalStartPosition;
    }
  });
}
